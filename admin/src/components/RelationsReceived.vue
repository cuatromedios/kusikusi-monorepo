...
export default {
  ...
  methods: {
    async getRelations () {
      this.loading = true
      const relationsResult = await this.$api.get(`/entities?relating=${this.entity.id}:${this.kind}&select=id,model,contents.title&only-published=false&order-by=relation.position&per-page=100`)
      this.loading = false
